...
**주요 결과** — 주방 오탐 88.0% → 1.3% (판별비 25배) · 웹3D 렌더 화면에서 **실사 픽셀 0장인 절차적 화염 85.0% 탐지**(정상 오탐 0%)
...
- [ ] 탐지 박스 위치 정확도(IoU) 측정 — 현재는 이미지 단위 유무만 측정
...
**웹3D 탐지의 박스 위치는 미검증**
채점이 이미지 단위 최대 신뢰도 기준 — 박스가 어디에 있든 탐지로 계수.
절차적 조건 정탐 중 화염이 아닌 영역에 박스가 잡힌 사례 확인.
화재 유무 판단에는 무해하나, **탐지 좌표를 VLM에 넘기는 파이프라인 5단계에서는 치명적.** IoU 기준 재측정 필요.
...
**1. 박스 위치 정확도 측정** — 웹3D 렌더는 화염 위치를 알고 있으므로 정답 박스 자동 생성 가능. IoU 0.5 기준으로 D를 다시 계산하면 파이프라인 5단계 진입 가능 여부가 판정됨.
