...
> **종결된 기록.** 2026-08-06, 프로젝트 주제 변경으로 10회차에서 실험을 종료함.
...
**5회차와 9회차는 학습하지 않은 급식실의 정상 조리에 반응하는 빈도가 실제 주방 화재에
반응하는 빈도보다 높았음.** 0.67과 0.62. 경보기로 성립하지 않는 상태였음.
10회차에서 **1.59** 로 처음 뒤집힘.

연기 라인을 미해결로 종결할 때 쓴 것과 같은 기준임. 화염 라인도 **학습하지 않은 급식실
...
11회차 negative 보강이 선택지가 아니라 유일한 경로임이 확인됨.

이후 5단계 로그에서 오탐의 정체가 더 좁혀짐 — 논현중은 노란 조리복 소매,
개원중은 흰 방진복임. 색은 정반대이나 둘 다 **주름진 밝은 천**이며 세로로 길쭉함.
11회차의 대상은 "조리복"이 아니라 **천 전반**이어야 했음.
**다만 주제 변경으로 11회차는 착수하지 않음.**
